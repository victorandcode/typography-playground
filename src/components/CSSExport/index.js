...
class CSSExport extends React.Component {
  ...
  getText() {
    const { fontFamily, fontSize, letterSpacing, lineHeight } = this.props.settings;
    const properties = [
  ...
  render() {
    return (
      <div className="CSSExport">
        <CopyToClipboard text={this.getText()} onCopy={() => this.triggerSuccess()}>
          <span>
            ...
            {this.state.showingCopyMessage && <span className="CSSExport__ctaActive">Copied!</span>}
          </span>
        </CopyToClipboard>
      </div>
    );
